...
1. Микроплан BrowserPool (3.2 + 3.3) → делегация haiku → коммит `feat: browser pool with round-robin + sticky binding`.
2. End-to-end тест: 3 последовательных new-chat запроса → каждый уходит в свой профиль, все 3 чата живут параллельно.
3. Триггер авто-саммари (искусственно занизить `TOKEN_LIMIT` в .env) → проверить что новый чат после саммари уходит на следующий профиль.
4. Только потом — Микроплан 3.4 (камуфляж B).
